...
function create_events_post_type()
{
    ...
    register_post_type('events', [
        'labels' => [
            'name' => 'Події',  // Загальна назва для цього типу запису
            'singular_name' => 'Подія',  // Одинична назва для цього типу запису
            'add_new' => 'Додати нову подію',  // Текст для кнопки додавання нового запису
            'add_new_item' => 'Додати нову подію',  // Текст для додавання нового елемента
            'edit_item' => 'Редагувати подію',  // Текст для редагування елемента
            'new_item' => 'Нова подія',  // Текст для нової позиції
            'view_item' => 'Переглянути подію',  // Текст для перегляду елемента
            'search_items' => 'Шукати події',  // Текст для пошуку
            'not_found' => 'Події не знайдено',  // Текст, що показується, коли немає записів
            'not_found_in_trash' => 'В кошику події не знайдено',  // Текст, що показується, коли в кошику немає записів
            'all_items' => 'Події',  // Текст для перегляду всіх елементів
            'archives' => 'Архів подій',  // Архіви типу запису
        ],
...
function render_events_main_meta_box($post)
{
    ...
    <div class="form-container-input_text">
        ...
        <input type="text" value="<?php echo esc_attr($input_text_city); ?>" name="input_text_city" id="input_text_city"
               class="input_text-item">
        <label for="input_text_location">Location</label>
        <input type="text" value="<?php echo esc_attr($input_text_location); ?>" name="input_text_location"
               id="input_text_location" class="input_text-item">

    </div>
    <div class="form-container-textarea">
        <label for="textarea_description">Description</label>
        <textarea name="textarea_description" id="textarea_description" cols="30"
                  rows="10"><?php echo esc_attr($textarea_description); ?></textarea>
